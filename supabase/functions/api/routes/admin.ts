...
export function registerAdminRoutes(app: Hono) {
  ...
  app.post("/admin/password-users", async (c) => {
    ...
    const usernameHash = await hashToken(normalizedUsername);
    ...
    `;
    return c.json({ user: { id, name: body.name, username: body.username, accessLevel: "ADMIN", mustChangePassword: true } }, 201);
  });
}
